...
def generate_all_12_itps():
    ...
    build_itp_pdf(
        ...
        items=[
            ...
            ['8', 'Kılavuzlu Bükme Testi (Guided-Bend)', 'Her test ünitesi için 1 set (1 kök + 1 kapak)', 'Enine kaynak dikişi bükme numunesi', 'ISO 5173 / ASTM A370', 'Mandrel Ag: 188.5 mm, Çene Bg: 220.3 mm; 180° bükmede kaynak/ITAB çatlak <= 3.2 mm', 'API 5L 9.9 / BOTAŞ 3.1.6', 'W'],
            ...
            ['10', 'Artık Stres Testi (Residual Stress Ring Test)', 'Her çap ve et kalınlığı ve HER DÖKÜMDE (HEAT) zorunlu', '150 mm genişlikte halka kesme', 'BOTAŞ Madde 3.3.9', 'S = (E*t*C) / (12.566*D^2) <= 0.10 x SMYS (Maksimum 45.0 MPa), Halka açılma <= 3.5 mm', 'Madde 3.3.9', 'H/W'],
            ...
            ['12', 'Kaynak Dikişi %100 NDT (Online UT + Offline RT)', 'Her boru tam boy kaynak dikişi (%100)', 'Online UT + Tamir/Hidro sonrası Offline RT', 'ISO 10893-11 / ISO 10893-6', 'Tam boy Online AUT (Seviye U2) + Uçlarda 200 mm ve tamirlerde RT Sınıf B', 'Madde 8.8.4.2 & 8.8.4.3', 'H/W'],
            ['13', 'Boru Gövdesi UT Laminasyon Muayenesi', 'Boru gövdesi yüzeyinin EN AZ %40 ı taranacak şekilde', 'Boru gövde yüzeyi', 'ISO 12094 B1 / BOTAŞ 8.8.4.4', 'Gövde yüzeyinin en az %40 ı taranacak, tekil alan <= 100 mm2 (ISO 12094 Sınıf B1)', 'Madde 8.8.4.4.1', 'W'],
            ['14', 'Boru Uçları Laminasyon NDT (UT)', 'İstisnasız her boru (%100) uç kısımları', 'Boru uçlarında asgari 50 mm genişlikte bant', 'ISO 10893-8 / ISO 12094', 'Boru uçlarında en az 50 mm çevre boyunca laminasyon kusuru bulunmayacaktır (Kusur <= 6.0 mm)', 'Madde 8.8.4.4.2', 'W'],
            ['15', 'Kaynak Ağzı ve Tamir Yüzeyi MT (Magnetic Particle)', 'Her boru (%100) kaynak ağızları ve tamirler', 'Boru uçları ve tamir kanalları', 'ISO 10893-5 / ASTM E709', 'Alın kaynak ağzı ve tamirlerde çatlak, katmer veya lineer kusur bulunmayacaktır', 'Madde 8.8.4', 'W'],
            ['16', 'Kaynak ve Gövde Tamir Kuralları (Repair Rules)', 'Yapılan her tamir işleminde', 'Tamir bölgesi ve kaynak dikişi', 'BOTAŞ Madde 9.1 & Ek C', 'Gövdeye kaynak YASAK; Tek tamir <= 150 mm; Uçta 300 mm tamir yasağı; Re-repair YASAK; Tamir sonrası %100 RT+MT ve Re-Hydro', 'Madde 9.1', 'H/W'],
            ['17', 'Kaynak Geometrisi, Radyal Kaçıklık ve Tepeleşme', 'Her boruda %100 dikiş boyunca', 'İç ve dış kaynak dikişi', 'BOTAŞ Çizelge 4 / API 5L Çizelge 16', 'Kaynak yüksekliği <= 2.625 mm | Radyal kaçıklık <= 1.125 mm | Tepeleşme <= 1.83 mm', 'Madde 8.8.4', 'W'],
            ['18', 'Dış Çap, Ovallik ve Geometrik Muayene', 'Her boruda %100 boyutsal ve dairesellik ölçümü', 'Boru gövdesi ve boru uçları', 'API 5L 10.2.8 / BOTAŞ 5.1', 'Gövde Çap: 1215.0 - 1223.0 mm (+-4.0 mm); Uç Ovalitesi <= 3.05 mm (BOTAŞ %50)', 'Madde 5.1 & 8.1.2', 'W'],
            ['19', 'Et Kalınlığı Ölçümü (Wall Thickness)', 'İstisnasız her boruda (%100 ultrasonik/ölçer)', 'Boru gövdesi ve boru uçları', 'API 5L Tablo 11 / BOTAŞ 5.2', 'Nominal: 14.30 mm, Kabul Aralığı: 13.16 - 16.45 mm (-%8.0 / +%15.0)', 'Madde 5.2 (Tablo 4)', 'W'],
            ['20', 'Boru Birim Ağırlığı ve Toleransı', 'Her boruda (%100 kantar tartımı)', 'Boru kantarı', 'API 5L Madde 9.11.2 & 10.2.8.7', 'Teorik Ağırlık: 424.87 kg/m | Münferit Boru: 410.00 - 467.36 kg/m (-%3.5 / +%10.0)', 'Madde 9.11.2', 'W'],
            ['21', 'Doğrusallık, Boy ve Alın Kaynak Ağzı', 'İstisnasız her boruda (%100)', 'Tam boy ve boru uçları', 'BOTAŞ Madde 5.4, 5.5, 7.2', 'Doğrusallıktan sapma <= %0.10 x L. Ağız açısı: 30° (+5°/-0°), Kök yüzeyi: 1.6 +- 0.8 mm, Diklik <= 1.6 mm', 'Madde 5.4 & 5.5', 'W'],
            ['22', 'Görsel Yüzey Muayenesi (Visual Inspection)', 'İstisnasız her boruda (%100 iç ve dış yüzey)', 'İç ve dış tüm boru yüzeyi', 'API 5L 10.2.7 / BOTAŞ 8.1.2', 'D >= 20" borularda %100 iç ve dış görsel kontrol; çatlak, katmer, kabuk yasaktır (Kusur <= %12.5 t)', 'Madde 8.1.2', 'W'],
            ['23', 'Kalıntı Manyetizma Ölçümü', 'Vardiyada EN AZ İKİ DEFA ve sevkiyat öncesi her boru', 'Boru uçları (çevresel 4 nokta)', 'API 5L 10.2.8.5 / BOTAŞ 8.1.1', 'Vardiyada min 2 kez ölçüm. Ortalama <= 3.0 mT (30 Gauss), Tekil <= 3.5 mT (35 Gauss)', 'Madde 8.1.1', 'W'],
            ['24', 'Proje Markalaması, Yüzey Temizliği ve MTC', 'Her boruda (%100)', 'Boru dış yüzeyi orta kısmı ve iç uçlar', 'BOTAŞ Madde 10 & 12', 'BOTAŞ / PROJE ADI / İMALATÇI şablonu + Sa 2.5 kum püskürtme (3LPE için) + EN 10204 Tip 3.1/3.2 MTC', 'Madde 10 & 12', 'W']
        ]
    )
